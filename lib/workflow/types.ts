    ...
    configSchema: [
      ...
      },
      {
        ...
        options: [
          ...
          { label: 'Delete Spreadsheet', value: 'deleteDocument' },
          { label: 'Append Row', value: 'append' },
          { label: 'Append or Update Row', value: 'appendOrUpdate' },
          { label: 'Clear Sheet', value: 'clear' },
          { label: 'Create Sheet', value: 'createSheet' },
          { label: 'Delete Sheet', value: 'deleteSheet' },
          { label: 'Get Row(s)', value: 'read' },
          { label: 'Update Row', value: 'update' },
          { label: 'Write Range', value: 'write' },
        ],
      },
      { key: 'spreadsheetId', label: 'Spreadsheet ID', type: 'text', placeholder: 'Use this for direct Google Sheets access' },
      { key: 'fileUrl', label: 'Google Sheet / CSV / XLSX Link', type: 'text', placeholder: 'https://docs.google.com/spreadsheets/d/... or Drive link' },
      { key: 'fileId', label: 'Drive File ID', type: 'text', placeholder: 'Optional alternative to link' },
      { key: 'title', label: 'New Spreadsheet Title', type: 'text', placeholder: 'Required for create' },
      { key: 'sheets', label: 'Sheet Tabs (JSON array)', type: 'json', placeholder: '["Sheet1", "Customers"]' },
      { key: 'sheetName', label: 'Sheet Name', type: 'text', placeholder: 'Sheet1' },
      { key: 'range', label: 'Range (e.g., Sheet1!A1:D10)', type: 'text', placeholder: 'For read/write. Leave empty to read a broad range.' },
      { key: 'columns', label: 'Columns / Mapping (JSON)', type: 'json', placeholder: '{"Name":"{{$json.name}}"}' },
      { key: 'values', label: 'Values (JSON, for write)', type: 'json', placeholder: '[["Name", "Email"], ["Ada", "ada@example.com"]]' },
      simpleOptionsField,
    ],
    inputs: [{ id: 'input', label: 'Input', type: 'any' }],
